...
function renderP0MavenTool(){
 ...
 refs.right.querySelectorAll("[data-maven-goal]").forEach(btn=>btn.addEventListener("click",()=>{state.maven.lastGoal=btn.dataset.mavenGoal;state.maven.status="BUILD SUCCESS";ensureMavenArtifacts(btn.dataset.mavenGoal);if(!state.visibleFeatures.includes("run"))state.visibleFeatures.push("run");activeBottom="run";setBottom("run","[INFO] --- "+btn.dataset.mavenGoal+"\n[INFO] BUILD SUCCESS");renderFeatureVisibility();renderP0MavenTool()}));
