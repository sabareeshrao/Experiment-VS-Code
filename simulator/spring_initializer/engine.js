...
window.addEventListener("message",e=>{const m=e.data||{};if(m.type==="SIM_PACKAGE"){baseline=normalize(m.package&&m.package.apps&&m.package.apps.spring_initializer||{});reset()}if(m.type==="SIM_SEEK"){reset();(m.steps||[]).forEach(apply);parent.postMessage({type:"SIM_SEEK_DONE",app:APP_ID},"*")}if(m.type==="SIM_PING")parent.postMessage({type:"ENGINE_READY",app:APP_ID,actions:SUPPORTED_ACTIONS},"*")});
